...
class MAMcontroller(QtCore.QObject):
    FILTER_MAP={
        'category': 'categories',
        'pattern': 'patterns',
        'sort': 'sort',
    ...
    def __init__(self, rootContext, window):
        super(MAMcontroller, self).__init__()
        self._window = window
    ...
    def search(self, rawData):
        ...
        filters[MAMcontroller.FILTER_MAP['pattern']] = \
            [value] if value else None

        value = rawData.property('category')
        filters[MAMcontroller.FILTER_MAP['category']] = \
            [value] if value else None

        filters[MAMcontroller.FILTER_MAP['source']] = \
            rawData.property('source') or None

        filters[MAMcontroller.FILTER_MAP['statuses']] = \
            rawData.property('status').split('|')[:-1] or None

        filters[MAMcontroller.FILTER_MAP['technical']] = \
            rawData.property('technical') or None

        searchData = {
            'filters': filters,
            'sort'   : MAMcontroller.SORT_MAP.get(rawData.property('sort'))
        }
    ...
    def _get_frontendDir(self):
        return frontend.MAM_FRONTEND_DIR
...
def start():
    mainQML = '%s/%s' % (frontend.MAM_QML_DIR,
    ...
    rc = view.rootContext()
    mam_controller = MAMcontroller(rc, window)
    rc.setContextProperty('mamController', mam_controller)

    view.setSource(mainQML)
    mam_controller.setRootObject(view.rootObject())
